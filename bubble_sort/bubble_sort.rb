...
def bubble_sort(test_array)
  ...
  until temp == true do
    temp = true
    (test_array.length - 1).times do |x|
      if test_array[x] > test_array[x.next]
        temp = test_array[x]
        test_array[x] = test_array[x.next]
        test_array[x.next] = temp
      end
    end
  end
  test_array
...
def bubble_sort_by(test_array)
  ...
  until temp == true do
    temp = true
    (test_array.length - 1).times do |x|
      if yield(test_array[x], test_array[x.next]) > 0
        temp = test_array[x]
        test_array[x] = test_array[x.next]
        test_array[x.next] = temp
      end
